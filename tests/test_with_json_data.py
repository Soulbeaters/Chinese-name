...
def _ensure_utf8_stdio():
    """Keep script-mode output UTF-8 without replacing pytest capture streams."""
...
def test_surname_database(firstname_json_path):
    assert run_surname_database_check(firstname_json_path).failed == 0


def test_name_order_detection(authors_json_path):
    assert run_name_order_detection_check(authors_json_path).failed == 0


def test_author_parsing(authors_json_path):
    assert run_author_parsing_check(authors_json_path).failed == 0


def test_edge_cases():
    assert run_edge_cases_check().failed == 0
