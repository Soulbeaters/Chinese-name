...
### Stage B4: true mention-to-profile candidate diagnostic

On 2026-07-02, `experiments/analyze_online_profile_candidate_ranking.py` evaluated true mention-to-profile candidates generated by the online benchmark, not sampled pairwise rows. It used the five existing online datasets and leave-one-dataset-out threshold/margin selection. The result is saved as `results/article2_online_profile_candidate_ranking_20260702.json`.

The best simple profile scorer was `combined_profile_graph_score`, but it still failed the low-UNKNOWN production gate:

| Held-out dataset | Precision | Recall | UNKNOWN | New-author false-link | Gate |
|---|---:|---:|---:|---:|---|
| Crossref ORCID | 99.538% | 89.630% | 9.953% | 1.184% | FAIL |
| Advisor DOI ORCID | 99.891% | 78.826% | 21.089% | 0.228% | FAIL |
| DBLP public | 99.797% | 82.261% | 17.571% | 0.432% | FAIL |
| LAGOS-AND public | 100.000% | 1.562% | 98.438% | 0.055% | FAIL |
| S2AND public | 100.000% | 0.709% | 99.291% | 0.000% | FAIL |

Conclusion: true profile-level features improve the diagnosis and show that the candidate/profile representation is the correct next layer, but a hand-built score is still insufficient. The next safe step is a supervised profile ranker with explicit new-author calibration, not another manual threshold tweak.
