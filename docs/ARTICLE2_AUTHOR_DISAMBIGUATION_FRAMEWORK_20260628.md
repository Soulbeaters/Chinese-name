...
- hypergraph fallback threshold ≥ 1.25
...
|---|---:|---:|---:|---|
| Crossref ORCID | 99.543% / 88.314% / 93.593% | +4.851 pp | 0.715%（333 / 46,548） | PASS |
| 导师 DOI ORCID | 99.904% / 89.413% / 94.368% | +11.488 pp | 0.259%（33 / 12,729） | PASS |
...
推荐使用统一最终验证入口，它会先运行 `pytest`、`compileall` 和补丁空白检查，再顺序重跑两套大规模数据上的 baseline、`framework_v1`、ISTINA proxy / hybrid 对比，并执行两个生产门槛：
