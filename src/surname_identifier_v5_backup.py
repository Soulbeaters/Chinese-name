...
def identify_surname_position(
    ...
    # Abbreviated format in scientific literature: "Liu W." / "Chen J.M." is always "Surname GivenInitial"

    def is_abbreviation_token(token):
        """检查是否为缩写 / Check if token is an abbreviation"""
        # 缩写特征：包含点号，去掉点号后长度<=3，且以字母开头
        # Abbreviation pattern: contains dot, length<=3 after removing dots, starts with letter
        if '.' not in token or not token[0].isalpha():
            return False
    ...
    # 检查所有非第一个词是否都是缩写
    # Check if all parts except first are abbreviations
    if len(parts) >= 2:
        other_parts = parts[1:]
        ...
        if all_abbreviations:
            # 检查第一部分是否为中文姓氏
            # Check if first part is Chinese surname
