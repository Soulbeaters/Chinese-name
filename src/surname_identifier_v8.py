...
def _apply_jmnedict_role_model(
    record: NameRecord,
    parsed: ParsedName,
    decision: NameDecision,
    cfg: SourceConfig,
) -> NameDecision:
    """Refine unresolved Crossref defaults using independent JMnedict roles."""
    if not get_ablation_config().enable_jmnedict_role_model:
        return decision
    if record.source not in ("CROSSREF", "crossref", "Crossref"):
        return decision
    if decision.confidence >= cfg.pub_conf_thresh:
    ...
    first = parsed.tokens[parsed.first_idx].ascii.lower()
    last = parsed.tokens[parsed.last_idx].ascii.lower()
    first_role = get_jmnedict_role(first)
    last_role = get_jmnedict_role(last)
    if not first_role or not last_role:
        return decision

    first_log_odds, first_support = first_role
    last_log_odds, last_support = last_role
    if first_support < 1 or last_support < 1:
        return decision
    delta = first_log_odds - last_log_odds
    if abs(delta) < 2.0:
        return decision
    ...
    return NameDecision(
        ...
        confidence=0.72,
        mode=decision.mode,
        reason_codes=decision.reason_codes + [f"JMNEDICT_ROLE_LOG_ODDS({delta:.2f})"],
    )


def _apply_ssa_census_role_model(
    record: NameRecord,
    parsed: ParsedName,
    decision: NameDecision,
    cfg: SourceConfig,
) -> NameDecision:
    """Refine unresolved Crossref defaults with normalized US role evidence."""
    if not get_ablation_config().enable_ssa_census_role_model:
        return decision
    if record.source not in ("CROSSREF", "crossref", "Crossref"):
        return decision
    eligible_reasons = {
    ...
    first = parsed.tokens[parsed.first_idx].ascii.lower()
    last = parsed.tokens[parsed.last_idx].ascii.lower()
    first_role = get_ssa_census_role(first)
    last_role = get_ssa_census_role(last)
    if not first_role or not last_role:
        return decision
    first_log_odds, first_support = first_role
    last_log_odds, last_support = last_role
    if first_support < 5 or last_support < 5:
        return decision
    delta = first_log_odds - last_log_odds
    if abs(delta) < 4.0:
        return decision
